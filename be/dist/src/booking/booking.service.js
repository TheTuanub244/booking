...
let BookingService = class BookingService {
  ...
  async cancelBooking(booking_id) {
    ...
    if (
      booking[0].booking_status === bookingStatus_enum_1.BookingStatus.PENDING
    ) {
      await this.bookingSchema.findByIdAndDelete(
        new mongoose_2.Types.ObjectId(booking_id),
      );
    } else if (
      booking[0].booking_status === bookingStatus_enum_1.BookingStatus.COMPLETED
    ) {
      if (
        booking[0].payment_status === paymentStatus_enum_1.PaymentStatus.PAID
      ) {
  ...
  async createBooking(createBookingDto) {
    ...
    const findExistedBooking = await this.bookingSchema
      ...
      .populate('user_id');
    if (
      findExistedBooking.booking_status ===
      bookingStatus_enum_1.BookingStatus.PENDING
    ) {
