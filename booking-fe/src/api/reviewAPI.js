...
export const getReviewByRate = async (property_id, minRate, maxRate, page) => {
  const respone = await axios.post(
    `${process.env.REACT_APP_API_URL}/review/getReviewByRate/${property_id}?page=${page}`, {min: minRate, max: maxRate}
  );
  return respone.data;
}
export const getReviewByType = async (property_id, review_type, page) => {
  const respone = await axios.post(
    `${process.env.REACT_APP_API_URL}/review/getReviewByType/${property_id}?page=${page}`, {review_type}
  );
  return respone.data;
}
