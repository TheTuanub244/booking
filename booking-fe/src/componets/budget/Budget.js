  ...
  return (
    <div className='budget'>
      ...
      <div className='pricingMode'>
        <button className={`btnPN ${activeButton === 'perNight' ? 'active' : ''}`} onClick={() => handleClickMode("perNight")}>Per night</button>
        <button className={`btnES ${activeButton === 'entireStay' ? 'active' : ''}`} 
      ...
      <div className='filterPrice'>
        <div className='status'>
          <span>VND {formatCurrency(values[0])} - VND {formatCurrency(values[1])}</span>
        </div>
